...
# 新增：基于fused_scores的损失函数
def fused_scores_loss(fused_scores, target_features, temperature=0.1):
    """
    利用fused_scores构造损失函数
    fused_scores: (n_samples, num_classes) 融合后的置信度分数
    target_features: (batch_size, feature_dim) 目标域特征
    """
    # 将fused_scores转换为伪标签概率分布
    pseudo_probs = torch.from_numpy(fused_scores).float().cuda()

    # 计算目标样本的softmax输出
    target_probs = F.softmax(target_features, dim=1)

    # KL散度损失：使模型输出接近伪标签分布
    kl_loss = F.kl_div(torch.log(target_probs + 1e-8), pseudo_probs, reduction='batchmean')
