...
def load_data_houston(image_file, label_file):
    with h5py.File(image_file, 'r') as f:
        data_all = np.array(f['ori_data']).T  # Transpose to match expected dimensions

    with h5py.File(label_file, 'r') as f:
        GroundTruth = np.array(f['map']).T
...
def load_data_pavia(image_file, label_file):
    ...
    # 更加健壮地寻找数据键值，排除元数据键
    data_keys = [k for k in image_data.keys() if not k.startswith('__')]
    label_keys = [k for k in label_data.keys() if not k.startswith('__')]
    
    if not data_keys or not label_keys:
        raise KeyError(f"Could not find data keys in {image_file} or {label_file}")
    ...
    print(f"Loaded {data_key} from {image_file}: {nRow}x{nColumn}x{nBand}")
